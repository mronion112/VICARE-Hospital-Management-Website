...
function Video({ title, sub, src, ok, update }: { title: string; sub: string; src?: string; ok: boolean; update: (n: number) => void }) { return <article className="video-card"><header><span><i /> <b>{title}</b><small>{sub}</small></span><Observe ok={ok} /></header><div>{src ? <video key={src} src={asset(src)} controls autoPlay muted loop playsInline onTimeUpdate={e => update(e.currentTarget.currentTime)} /> : <p>Đang tải video...</p>}<aside><span><i /> LIVE DATASET</span><small>PHÒNG 101</small></aside></div></article> }
...
function Temperature({ value }: { value: number }) { return <article className="ground temperature"><header><i style={{ color: "#d88928" }}><Icon name="camera" /></i><span><h3>Nhiệt độ bề mặt</h3><p>L_CAS Thermal · ước tính</p></span><strong>{value.toFixed(1)}<small>°C</small></strong></header><div className="temperature-note"><span>Đo từ vùng mặt/thân trên video nhiệt</span><small>Không phải ground-truth hiệu chuẩn</small></div></article> }

function PatientPage({ patient, monitor, select }: { patient: Patient; monitor?: Monitor; select: (b: string) => void }) {
  const [lt, setLt] = useState(0), [it, setIt] = useState(0), ok = patient.state !== "blocked"
  const bvp = monitor?.imvia.bvp.reduce((a, b) => Math.abs(b[0] - it) < Math.abs(a[0] - it) ? b : a, monitor.imvia.bvp[0] || [0, 0])?.[1] || 0
  return <div className="vc-page monitor-page"><div className="monitor-head"><div><span>GIƯỜNG {patient.bed} · PHÒNG 101</span><section><h1>{patient.name}</h1><Badge state={patient.state} /></section><p>{patient.code} · {patient.gender}, {patient.age} tuổi · {patient.diagnosis}</p></div><nav>{PATIENTS.map(p => <button className={p.bed === patient.bed ? "active" : ""} key={p.bed} onClick={() => select(p.bed)}>{p.bed}<i className={p.state} /></button>)}</nav></div>
    ...
    <div className="patient-layout"><section className="patient-main"><div className="videos"><Video title="Camera nhiệt L_CAS" sub={`Thermal · ${monitor?.lcas.subject || "Đang tải"}`} src={monitor?.lcas.video} ok={ok} update={setLt} /><Video title="Camera IMVIA-FIR" sub={`Far-infrared · Subject ${monitor?.imvia.subject || "--"}`} src={monitor?.imvia.video} ok={ok} update={setIt} /></div><div className="ground-title"><span><small>GROUND-TRUTH & CHỈ SỐ ƯỚC TÍNH</small><h2>Dữ liệu sinh lý đồng bộ</h2></span><em><Icon name="pulse" size={15} /> Đang chạy theo video</em></div><div className="ground-grid"><Ground title="Nhịp tim" source="L_CAS · heartbeat.csv" value={monitor ? String(Math.round(monitor.lcas.heartRate)) : "--"} unit="BPM" icon="heart" points={monitor?.lcas.heartbeat || []} time={lt} color="#e75264" /><Ground title="Nhịp thở" source="L_CAS · respiration.csv" value={monitor ? String(monitor.lcas.respirationRate) : "--"} unit="/phút" icon="pulse" points={monitor?.lcas.respiration || []} time={lt} color="#0ca582" /><Ground title="Tín hiệu BVP" source="IMVIA-FIR · BVP.csv · Blood Volume Pulse" value={monitor ? bvp.toFixed(1) : "--"} unit="a.u." icon="pulse" points={monitor?.imvia.bvp || []} time={it} color="#1687d9" /><Temperature value={patient.temperature} /></div></section>
